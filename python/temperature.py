...
#
# Purpose           : Plot temperature images as part of SWIFT_GFSplotting.
#
# Revision History  :
#
# Usage             : Can be used as part of wider plotting repository or independently e.g.
#                     "python3 temperature.py time lev lat lon lat lon"
...
wks_type = "png"
wks = ngl.open_wks(wks_type, "GFSanalysis_%s_%s_temperature_%shPa" % (region, init_dt[0:10], lev_hPa))
...
res.lbTitleString         = "%shPa temperature" % (lev_hPa)
...
# open workspace for forecast plots
   ...
   wks_type = "png"
   wks = ngl.open_wks(wks_type, "GFSforecast_%s_%s_temperature_%shPa_%s_%03d" % (region, valid_date, lev_hPa, init_dt[0:10], fore[i]))
...
os.system('mv *_'+region+'_'+init_dt[0:10]+'_temperature_'+lev_hPa+'hPa.png MARTIN/GFS/'+region+'/'+init_dt[0:10]+'/temperature_'+lev_hPa)
...
os.system('mv *'+region+'_*temperature_'+lev_hPa+'hPa_'+init_dt[0:10]+'*.png MARTIN/GFS/'+region+'/'+init_dt[0:10]+'/temperature_'+lev_hPa)
